...
public struct WooCommerceProduct: Decodable, Identifiable, Hashable, Equatable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        do {
            ...
            if let stringValue = try? container.decodeIfPresent(String.self, forKey: .total_sales) {
                total_sales = Int(stringValue)
            } else { total_sales = nil }
        } catch { total_sales = nil }
    }
    
    public static func == (lhs: WooCommerceProduct, rhs: WooCommerceProduct) -> Bool { lhs.id == rhs.id }
...
// MARK: - Product Sub-Models (gehören logisch zu Produkten)
